...
\begin{align}
  \cos(2\pi f t)\;& 2\cos(2 \pi f_0 t) = \frac{1}{2} \bigg( e^{i 2\pi f t} + e^{-i 2 \pi f t} \bigg)\bigg(e^{i 2\pi f_0 t} + e^{-i 2 \pi f_0 t} \bigg) \\\\  &= \frac{1}{2}\bigg[ e^{i 2\pi (f+f_0)t} + e^{i 2\pi (f-f_0)t} + e^{-i 2\pi (f-f_0)t} + e^{-i 2\pi (f+f_0)t}\bigg] \\\\ &= \cos[2\pi(f+f_0)t] + \cos[2\pi(f-f_0)t]
\end{align}
We get a cosine at the sum of the two frequencies and another at the difference frequency. If $$f$$ and $$f_0$$ are not too dissimilar, the difference frequency will oscillate **much** more slowly, so that if we were to pass this product of cosines through a low-pass filter, only the difference frequency would emerge with appreciable amplitude.
